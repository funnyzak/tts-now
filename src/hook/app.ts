...
const getVoiceTypeList = (_engine?: TtsEngine) => voiceData[
  _engine
    ? _engine.toString()
    : SettingContext().appSetting.ttsSetting.engine.toString()
];
...
const currentSpeaker = (_speakerId?: string, _engine?: TtsEngine) => {
  ...
  const _findSpeakers = voiceTypeList.filter(
    (v) => v.speakerId === speakerId
  );
